...
description: Use when after applying biotransformation rules to generate candidate
  product structures from input molecules, when the same transformed structure can
  be produced via multiple transformation pathways or rule applications, and you need
  a single canonical representation while tracking which rules and.
license: CC-BY-4.0
metadata:
