...
3. **Run the propose-skill flow (normalize → match → ground → tier).** Follow
   ...
   - match: `scripts.skill_match.match_skills` / `match_tools` over the
     collection's `skills_index.json` + `tools_index.json` to fill
     `related_skills` + `tools_used`, and — separately, on its own tool-free
     scale — `near_duplicates(duplicate_candidates(prose, col),
     threshold=DUPLICATE_THRESHOLD)` for the duplicate question (warn + suggest
     annotate/merge on a near-dup; never refuse on it);
...
7. **Print the review summary + the exact PR commands — then stop.** Show: the
   ...
   python scripts/tier_update.py --orcid "<author-ORCID>" \
       ...
       --criteria "<the collection's curator-criteria.yaml>"
   ```
   (The criteria file is per-collection and generated from
   [`templates/curator-criteria.yaml.template`](../../../../templates/curator-criteria.yaml.template);
   `metabolomics/v2` does not ship one yet, so pass the path the collection
   actually has rather than assuming `collections/<slug>/<vN>/`.)
