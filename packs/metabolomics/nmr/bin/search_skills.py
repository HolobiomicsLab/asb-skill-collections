#!/usr/bin/env python3
# Generated selector: edit asb_skill_collections/asb_skill_index.py, then run
# scripts/router_shape.py --scripts-only. The CLI wrapper below is maintained here.
# BEGIN VENDORED SELECTOR
...
# END VENDORED SELECTOR

# Wrapper imports follow the verbatim module, including its future import.
...
def load_index(collection: Path, target: str) -> list[dict]:
    """Read a published index through the shared target resolver."""
    path = index_path(collection, target)
    if not path.is_file():
        raise FileNotFoundError(f"this collection publishes no {target} ({path} is absent)")
    return load_rows(collection, target)


def read_dir(collection: Path, target: str) -> str:
    """Return the absolute source directory, usable from any working directory."""
    base = unit_root(collection)
    if target == "workflows":
        return str(index_path(base, target).parent)
    if target == "tools":
        return str(base / "tools")
    return str(base / ("leaves" if (base / "leaves").is_dir() else "skills"))


def format_hit(rank: int, entry: dict, leaf_dir: str, target: str = "skills") -> str:
    """Render a candidate, its source path and the same typed JSON explanation."""
    tools = ", ".join(entry_tools(entry)[:6])
    desc = " ".join(str(entry.get("description") or "").split())
    stages = entry.get("stages") or []
    scope = f"stages: {' -> '.join(str(stage) for stage in stages)}" if stages else (
        f"tier: {entry.get('license_tier') or '-'}")
    suffix = f"{entry['slug']}.yaml" if target == "tools" else f"{entry['slug']}/SKILL.md"
    explanation_keys = ("selector", "collection", "target", "slug", "qualified_slug",
                        "matched_fields", "score", "filters", "fallback")
    explanation = {key: entry[key] for key in explanation_keys if key in entry}
    return (
        f"{rank}. {entry.get('slug')}\n"
        f"   {desc}\n"
        f"   tools: {tools or '-'} | techniques: "
        f"{', '.join(entry.get('techniques') or []) or '-'} | {scope}\n"
        f"   read: {leaf_dir}/{suffix}\n"
        f"   explanation: {json.dumps(explanation, ensure_ascii=False, sort_keys=True)}"
    )


def build_parser() -> argparse.ArgumentParser:
    """CLI for the canonical offline selector; JSON is shared with CLI/MCP."""
    parser = argparse.ArgumentParser(description="Search collection skills, workflows or tools offline.")
    parser.add_argument("--query", default="", help="free text describing the task")
    parser.add_argument("--collection", default=str(COLLECTION_ROOT))
    parser.add_argument("--target", choices=sorted(TARGETS), default="skills")
    parser.add_argument("--technique", help="exact technique tag, e.g. LC-MS")
    parser.add_argument("--tool", help="exact tool name, e.g. SIRIUS")
    parser.add_argument("--edam", help="substring of an EDAM operation/topic IRI")
    parser.add_argument("--selector", choices=SELECTORS,
                        help="scoring rule (or ASB_SELECTOR_RULE); router is deprecated for one release")
    parser.add_argument("--max-tools", type=int, help="maximum tools for skills; 0 disables the guard")
    parser.add_argument("-k", "--k", type=int, default=10, help="how many candidates to print")
    parser.add_argument("--json", action="store_true", help="print the shared result dictionaries")
    return parser


def main(argv: list[str] | None = None) -> int:
    """Search a unit and print candidates; return 1 for no match, 2 for bad input."""
    args = build_parser().parse_args(argv)
    collection = unit_root(args.collection)
    try:
        rows = load_index(collection, args.target)
        hits = keyword_search(
            rows, args.query, args.technique, args.k, args.max_tools,
            collection=collection_id(collection), target=args.target,
            selector=args.selector, tool=args.tool, edam=args.edam,
        )
    except (OSError, ValueError) as error:
        print(f"error: {error}", file=sys.stderr)
        return 2
    if args.json:
        print(json.dumps({"mode": "keyword", "target": args.target,
                          "query": args.query, "results": hits}, indent=2))
    elif hits:
        by_slug = {row["slug"]: row for row in rows}
        where = read_dir(collection, args.target)
        for rank, hit in enumerate(hits, 1):
            print(format_hit(rank, {**by_slug[hit["slug"]], **hit}, where, args.target))
    else:
...
def selftest() -> None:
    """Check standalone keyword routing and tool-only workflow retrieval."""
    sample = [
        {"slug": "peak-align", "name": "Peak alignment",
         "description": "Align LC-MS peaks across samples", "tools": ["XCMS"],
         "techniques": ["LC-MS"]},
        {"slug": "formula-id", "name": "Formula prediction", "tools": ["SIRIUS"],
         "description": "Predict molecular formulas from MS2", "techniques": ["LC-MS"]},
    ]
    assert keyword_search(sample, "align peaks across samples")[0]["slug"] == "peak-align"
    assert [hit["slug"] for hit in keyword_search(sample, "", tool="SIRIUS")] == ["formula-id"]
    assert keyword_search(sample, "", technique="GC-MS") == []
    flows = [{"slug": "untargeted", "name": "Untargeted pipeline",
              "member_tools": ["MZmine", "SIRIUS"], "stages": ["preprocess", "annotate"]}]
    assert keyword_search(flows, "MZmine", target="workflows")[0]["slug"] == "untargeted"
